...
(function () {
    ...
    if (angular) {
        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            // $rootScope, $window, $q, $location, $timeout, $injector are injected by Angular
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var loginHandler = function (loginStartPage) {
                    ...
                    if (_adal.config && _adal.config.localLoginUrl) {
                        ...
                        _adal._saveItem(_adal.CONSTANTS.STORAGE.START_PAGE, loginStartPage || $location.$$path);
                ...
                var stateChangeHandler = function (e, nextState, nextStateParams) {
                    if (nextState && isADLoginRequired(nextState, _adal.config)) {
                        if (!_oauthData.isAuthenticated) {
                            var nextURL = getStateFullURL(nextState, nextStateParams);
                            _adal.info('State change event for:' + $location.$$path);
                            loginHandler(nextURL);
                        }
                    }
                };
                
                var getStateFullURL = function (state, nextStateParams) {
                    var stateService = null;
                    try {
                        stateService = $injector.get('$state');
                    } catch (e) {
                        _adal.warn('Could not get $state service:' + e.message);
                    }
                    if (stateService) {
                        var url = stateService.href(state.name, nextStateParams || {});
                        if (url.indexOf('#') === 0) {
                            url = url.slice(1);
                        }
                        return url;
                    ...
                    acquireToken: function (resource) {
                        ...
                        _adal.acquireToken(resource, function (error, tokenOut) {
                            if (error) {
                                ...
                                if (error === _adal.CONSTANTS.ERR_MESSAGES.TOKEN_REFRESH_TIMED_OUT)
                                {
                                    $rootScope.$broadcast('adal:tokenRefreshTimedOut');                                    
                                }
            ...
            return {
                request: function (config) {
                    if (config) {
                        ...
                        // do not run for items in templatecache
                        ...
                        if (tokenStored) {
                            ...
                            if (authService.loginInProgress()) {
                                // you'll get a route change error when you cancel the request.
                                // this helps filter out such errors from the log
                                canceller.resolve('adal:requestCancelledLoginInProgress');
                            } else if (authService.config && (isEndpoint || resource === authService.config.loginResource)) {
                                // external endpoints
