...
AuthenticationContext = function (config) {
    ...
    */
    ...
    if (!this.config.tokenRefreshTimeout) {
        this.config.tokenRefreshTimeout = this.CONSTANTS.TOKEN_REFRESH_TIMEOUT;
    }
...
AuthenticationContext.prototype.registerCallback = function (expectedState, resource, callback) {
    ...
    if (!window.callBackMappedToRenewStates[expectedState]) {
        ...
        // there will be times when iframe will silently fail to load (x-frame-options for example)
        // in these cases we will want to have a timeout period after which we can assume that
        // token refresh has failed
        setTimeout(function() {
            // check if we are still waiting for the same state to come back or it's a different renewal request
            if (self._activeRenewals[resource] === expectedState) {
                for (var i = 0; i < window.callBacksMappedToRenewStates[expectedState].length; ++i) {
                    window.callBacksMappedToRenewStates[expectedState][i](self.CONSTANTS.ERR_MESSAGES.TOKEN_REFRESH_TIMED_OUT, '');
                }
                self._activeRenewals[resource] = null;
                window.callBacksMappedToRenewStates[expectedState] = null;
                window.callBackMappedToRenewStates[expectedState] = null;
            }
        }, self.config.tokenRefreshTimeout);
    }
};
